...
export const orderUtils = {
    ...
    compareBidOrderEntity: (orderA: SignedOrderV4Entity, orderB: SignedOrderV4Entity): number => {
        return orderUtils.compareBidOrder(
          orderUtils.deserializeOrderToSRAOrder(orderA as Required<SignedOrderV4Entity>).order,
          orderUtils.deserializeOrderToSRAOrder(orderB as Required<SignedOrderV4Entity>).order
        )
    },
    compareAskOrderEntity: (orderA: SignedOrderV4Entity, orderB: SignedOrderV4Entity): number => {
        return orderUtils.compareAskOrder(
          orderUtils.deserializeOrderToSRAOrder(orderA as Required<SignedOrderV4Entity>).order,
          orderUtils.deserializeOrderToSRAOrder(orderB as Required<SignedOrderV4Entity>).order
        )
    },
    ...
    deserializeOrder: (
        signedOrderEntity: Required<SignedOrderV4Entity>,
    ): SignedLimitOrder => {
    ...
    serializePersistentOrder: (apiOrder: SRAOrder): SignedOrderV4Entity => {
        const signedOrder = apiOrder.order;
        const persistentOrder = new SignedOrderV4Entity({
            signature: orderUtils.serializeSignature(signedOrder.signature),
            sender: signedOrder.sender,
            maker: signedOrder.maker,
            taker: signedOrder.taker,
    ...
    getOrderHash: (order: LimitOrder): string => {
        order.maker = '0x' + Buffer.from(Base58.decode(order.maker).buffer).toString('hex');
