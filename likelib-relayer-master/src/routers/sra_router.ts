...
export function createSRARouter(orderBook: OrderBookService): express.Router {
    ...
    /**
     * POST Order endpoint cancels the order by order hash
     */
    router.post('/cancel', asyncHandler(handlers.cancelOrderAsync.bind(handlers)));
    router.get('/order-seed', asyncHandler(handlers.seedDatabase.bind(handlers)));
    return router;
}
